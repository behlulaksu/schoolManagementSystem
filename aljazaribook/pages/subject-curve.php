<?php /* Template Name: Subject Curve */ ?>
		...
		<div class="container-fluid px-[0.625rem]">
			...
			if ($my_posts->have_posts()) {
				while ($my_posts->have_posts()) {
					...
					/************************************/  
					$student_group_marge = get_field("group_users",get_the_id());
					for ($i=0; $i < count($student_group_marge); $i++) { 
						$student_group_marge[$i]["student_grade"] = get_the_id();
					}
					$group_users = array_merge($group_users,$student_group_marge);
				}
			}
			wp_reset_query();
			...
			//$group_users = get_field("group_users",$all_groups[0]);
						...
						while(have_rows('add_sub_domains')): 
							...
							foreach ($group_users as $key => $value) {
								...
								foreach ($subject_all_marks as $keyler => $valueler) {
									if ($valueler->gb_group_id == $group_users[$key]['student_grade'] && $valueler->gb_gradebook_id == $gradebook_ID && $valueler->gb_domain_id == $data_id_counter && $valueler->gb_subdomain_id == $subDomainID && $valueler->gb_student_id == $value['ID']) {
					...
					<div class="items-center p-4 border-b rounded-t border-gray-50 dark:border-zinc-600">
						<div class="flex" style="justify-content: space-around;">
							<h3 style="text-align: center;" class="text-xl font-semibold text-gray-900 dark:text-gray-100 ">
								<?php echo get_the_title($subject); ?>
							</h3>
							<h3 style="text-align: center;" class="text-xl font-semibold text-gray-900 dark:text-gray-100 ">
							...
							$query = $wpdb->prepare("SELECT * from $bg_table_name where blog_id =".$blog_id." and quarter_id = ".$quarter." and class_id = 8888888888 and subjecet_id =".$subject."" );
								...
								<tbody>
									...
									foreach ($group_users as $key => $value) {
										if (!empty($value['ID'])) {
											...
											<script>
												...
												students_grades[<?php echo $key; ?>] = <?php echo $value['student_grade']; ?>;
											</script>
									...
									<tr class="bg-white border border-gray-50 dark:border-zinc-600 dark:bg-transparent">
										...
										<th scope="row" class="px-6 py-3.5 border-l border-gray-50 dark:border-zinc-600 font-medium text-gray-900 whitespace-nowrap dark:text-zinc-100 student_id">
											...
											$query = $wpdb->prepare("SELECT * from $bg_table_name where bloog_id =".$blog_id." and group_id =".$group_users[0]['student_grade']." and subjecet_id =".$subject." and quarter_id = ".$quarter."" );
...
<script>
	$(document).ready(function(){
		$("#header_title").text("<?php echo get_the_title($subject); ?> - Curve");
	});
	...
	$(".save_avarages").click(function(){
		...
		var value = $.ajax({
			method: "POST",
			url: get_site_url+'/wp-admin/admin-ajax.php',
			data: ({action:'my_subject_avarage',
